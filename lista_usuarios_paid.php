...
<SCRIPT LANGUAGE="JavaScript">
  ...
  function buscar() {
	...
	document.forma.action='lista_usuarios_paid.php';
  ...
  function ir(form, Pag) {
    ...
    form.action='lista_usuarios_paid.php';
	form.target='_self';
    form.submit();
  }
...
<?
   ...
   if (empty($ord)) $ord='nextPaymentDate';

   if     ($ord=='nombre') $orden='ORDER BY usuario.nombre,usuario.apellidos';
   elseif ($ord=='nextPaymentDate') $orden='ORDER BY nextPaymentDate';
...
//    elseif ($ord=='email') $orden='ORDER BY usuario.email';
   elseif ($ord=='salesperson') $orden='ORDER BY salesperson, nextPaymentDate';
   else $orden='order by nextPaymentDate';
...
<img src="../images/icon-logout.png" width="16" height="11" alt="">&nbsp;Salir</a>
	 	 ...
	 	 <div class="contact">
			 	 ...
			 	 <div class="content">
		 	 	<h2 class="style">Paid User</h2>

                <br>
                <p>Criterios de b&uacute;squeda</p>
		 	 </div>
             <br>
			 <form action="lista_usuarios_paid.php" method="post" name="forma" id="forma">
...
<!-- <th >File Size</th> -->
                  ...
                  <th >Visitas</th> -->
                  <!-- <th >Sales Person</th> -->
				  <th><a href="javascript:ordena('salesperson');" style="color:#FFF;">Sales Person</a><? if ($ord=='salesperson') echo ' <img src="../images/orden.png" width="15" height="15" align="absmiddle">'; ?></th>
                  <th >Payment</th>
                  <!-- <th >Activo</th>
                  <th >Opciones</th> -->
                  <th >Note</th>
...
*/
// SELECT * FROM usuario, tracking_Payment WHERE 1=1 and usuario.clave=tracking_Payment.userid ORDER BY usuario.nombre,usuario.apellidos LIMIT 0,10;
				$currentDate = date('Y-m-d');
				$sql = "SELECT * FROM usuario, tracking_Payment $condicion and usuario.clave=tracking_Payment.userid and nextPaymentDate>='$currentDate' and nextPaymentDate IS NOT NULL $orden LIMIT $regini,$ver";
			   $resultado= mysql_query($sql, $conexion);
			//    $resultado= mysql_fetch_array( $resultado );
			//    var_dump($sql);
			//    exit();
			//    echo "Subroto";
			//    $sql = "SELECT * FROM usuario, tracking_Payment $condicion and usuario.clave=tracking_Payment.userid $orden LIMIT $regini,$ver";
			//    var_dump($sql);
			//    var_dump($conexion);
...
<td> <!-- Sales person block  -->
	<?php
		// $sql = "SELECT nombre, apellidos FROM usuario WHERE email='$userEmail'";
		$sql = "SELECT nombre, apellidos, salesperson FROM usuario, tracking_Payment WHERE usuario.email=tracking_Payment.salesPerson and userid=$usuario";
		$salesPerson= mysql_query( $sql, $conexion );
		$salesPerson= mysql_fetch_array($salesPerson);
		// var_dump($sql, $salesPerson); exit();
		if ($salesPerson['salesperson'] != NULL) {
			$fullName = $salesPerson['nombre'] . " " . $salesPerson['apellidos'];
			echo "<p>$fullName</p>";

			echo "Email: " . $salesPerson['salesperson'];
			echo '<input type="text" style="width: 100%;" name="salesperson" data-userid="'. $usuario .'" id="salesperson" value="'. $salesPerson['salesperson'] .'">';
			echo '<p class="savedSalesperson'. $usuario .'" style="color: green;"></p>';
		} else {
			echo '<input type="text" style="width: 80%;" name="salesperson" data-userid="'. $usuario .'" id="salesperson" value="'. $salesPerson['salesperson'] .'">';
			echo '<p class="savedSalesperson'. $usuario .'" style="color: green;"></p>';
		}
	?>
</td>

<td class="paymentStatus-<?= $usuario ?>">
	<?php 
		...
		if( $paymentStatus['nextPaymentDate'] !=null ) {
			...
			echo "<a href='aaa.php?id=". $usuario ."' rel='modal:open' class='paymentDetails'>Paid On: " . $new_paymentDate . "</a>";
			$date1=date_create( date('Y-m-d') );
			$date2=date_create( $paymentStatus['nextPaymentDate'] );

			$dayDifferent = date_diff($date1, $date2)->days;
			$dayCount = date_diff($date1, $date2)->days < 2 ? "day" : "days";
			echo "<p class='redcolor'><span style='font-weight: bold;'>" . $dayDifferent . "</span> " . $dayCount . " to next payment <br></p>";
			echo '<a id="backToFree" class="backToFree-'. $usuario .'" data-userid="'. $usuario .'" href="#">Back to Free</a>';
		} else {
			echo '<a id="activePayment" class="notWorkingpaymentDetails<?= $usuario; ?>" data-userid="<?php echo $usuario ?>" href="#">Active Payment</a>';
		}
	?>
...
<td>
	<textarea id="w3review" name="w3review" rows="4" cols="30" data-userid="<?php echo $usuario ?>"><?php echo $paymentStatus['note']; ?></textarea>
...
<!-- Normal link -->
<a href="aaa.php" rel="modal:open">example</a>
...
<!------subroto-19-feb-2014-start---------->
 ...
 <!-- https://jquerymodal.com/ jQuery Modal -->
<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery-modal/0.9.1/jquery.modal.min.js"></script>
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/jquery-modal/0.9.1/jquery.modal.min.css" />
 <script>
	// Active Payment if Document is OK
	...
	$("textarea#w3review").blur(function(){
		...
		var notificationClass= "saved" + values;
		var url = "saveNote.php?id=" + values + "&note=" + noteValue;
	...
	// salesperson Email update dinamic
	$("input#salesperson").blur(function(){
		var values = $(this).data('userid');
		// alert(values);
		var salesPersonEmail= $(this).val();
		var notificationClass= "savedSalesperson" + values;
		var url = "activePayment.php?id=" + values + "&salesPersonEmail=" + salesPersonEmail;
		// alert(notificationClass);
		// alert(url);
		if(salesPersonEmail == "") {
			exit();
	...
	// Back to Free dinamic
	$( document ).on( 'click', 'a#backToFree', function(evt) {
	evt.preventDefault();
	var values = $(this).data('userid');
	var url = "activePayment.php?backToFreeUserID=" + values;
	var backToFreeClass = this.className;
	var newHideClassName = "td.paymentStatus-" + values + " p";

	$.get( url, function( data ) {
		$('#backToFree').hide();
		$(newHideClassName).hide();
		$( "." + backToFreeClass ).show( );
		$( "." + backToFreeClass ).html( data );
	});
	});
